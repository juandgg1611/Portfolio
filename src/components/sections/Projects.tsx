        ...
        {projects.map((project, index) => (
          ...
          >
            <div className="p-3 pb-0 flex justify-center">
              <div
                className="mc-img-wrap relative overflow-hidden rounded-2xl w-full"
                style={{
                  aspectRatio: project.isMobile ? '3 / 4' : '1919 / 923',
                  maxWidth: project.isMobile ? '280px' : '100%',
                  clipPath: 'inset(100% 0 0 0 round 14px)',
                  background: project.isMobile ? 'rgba(255,255,255,0.05)' : 'transparent',
                }}
              >
                <Image
...
export default function ProjectsPage() {
  ...
  const handleRowMouseEnter = (e: React.MouseEvent<HTMLAnchorElement>, imageUrl: string, index: number) => {
    ...
    setHoveredProject(projects[index] ?? null);
  ...
  return (
    ...
    >
      {/* ═══════════════════════════════════════
          DESKTOP
      ══════════════════════════════════════ */}
      <div className="hidden md:block py-24 md:py-32 px-6 sm:px-8 md:px-12 lg:px-16 max-w-[1400px] mx-auto">
        <div className="mb-12">
          <AnimatedHeading
            text="PROYECTOS"
            className="text-[clamp(2rem,5vw,4.5rem)] font-black leading-none uppercase text-charcoal"
          />
        </div>
        ...
        <div className="flex flex-col w-full">
          {projects.map((project, index) => (
            <Link
              key={project.id}
              href={`/projects/${project.slug}`}
              className="project-row-desktop relative flex items-stretch border-b border-border py-8 min-h-[140px] group cursor-pointer no-underline"
              onMouseEnter={(e) => handleRowMouseEnter(e, project.hoverImage || project.images[0], index)}
              onMouseLeave={handleRowMouseLeave}
              data-cursor="view"
              onClick={handleRowClick}
            >
              <div className="flex-[0_0_80px] font-mono text-xl sm:text-2xl text-gray-soft relative h-8 overflow-hidden mt-2">
              ...
              <div className="flex-1 pr-8">
                <h3 className="relative text-[clamp(2.5rem,5vw,4.5rem)] font-extrabold uppercase leading-none tracking-tight overflow-hidden pb-1">
                ...
                <div className="mt-3 flex flex-wrap gap-2 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-out">
                  {project.tech.map((t) => (
                    <span key={t} className="px-4 py-1.5 rounded-full bg-transparent border border-gray-300 text-warm text-base font-medium">
                      {t}
                    </span>
                  ))}
                </div>

                <div className="font-mono text-base text-gray-soft mt-4">{project.year}</div>
              </div>

              <div className="flex-[0_0_240px] text-right flex flex-col justify-end items-end pb-4">
                <span className="font-mono text-base uppercase tracking-widest text-charcoal group-hover:text-forest transition-colors duration-250 flex items-center gap-2">
        ...
        {/* ── Email row ── */}
        <div className="flex items-center justify-between border-t border-border pt-8 pb-4 mt-2">
          <span className="font-mono text-sm uppercase tracking-widest text-gray-soft">¿Tienes algo en mente?</span>
          <a
            href="mailto:juandgg11o@gmail.com"
            className="font-mono text-base md:text-lg uppercase tracking-widest text-charcoal hover:text-forest transition-colors duration-300 flex items-center gap-2 group"
          >
            <span>juandgg11o@gmail.com</span>
          ...
          {hoveredProject?.isMobile ? (
            ...
            >
              ...
              <div style={{
                ...
                boxShadow: '0 0 0 1px rgba(255,255,255,0.08), inset 0 1px 0 rgba(255,255,255,0.06), 0 24px 60px -8px rgba(0,0,0,0.9)',
                overflow: 'hidden',
                zIndex: 2,
                pointerEvents: 'none',
                padding: '6px',
                ...
                <div style={{
                  ...
                  <div
                    ...
                    style={{ willChange: 'transform', transform: 'scale(1.03)' }}
                  >
              ...
              <div style={{
                position: 'absolute', left: '-3px', top: '30%',
                width: '3px', height: '22px',
                background: 'linear-gradient(180deg, #2a2a2a, #1a1a1a)',
                borderRadius: '2px 0 0 2px',
              }} />
      ...
      {/* ── Email row (mobile) ── */}
      <div className="flex flex-col gap-2 px-6 py-8 border-t border-border md:hidden">
        <span className="font-mono text-xs uppercase tracking-widest text-gray-soft">¿Tienes algo en mente?</span>
        <a
          href="mailto:juandgg11o@gmail.com"
          className="font-mono text-sm uppercase tracking-widest text-charcoal hover:text-forest transition-colors duration-300"
        >
